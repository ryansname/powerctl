...
9. **dynamicInverterControl** (src/dynamic_inverter_control.go) - Actively controls Multiplus II (Battery 3) setpoint every 5s. Range: -3000W to +3500W.
   ...
   - **CCL headroom**: keeps total battery current `CCL − 5A` in both directions via `headroomA = (CCL − 5) − (solar3A + solar4A)`, converted to watts `× voltage`. headroomA<0 (solar over the limit) forces Multiplus discharge of the excess to relieve MPPT throttling; headroomA>0 caps Multiplus MaxCharge at the remaining room so solar + charge stays under the limit. Stateless, no ramp; no-op at 0V. Safety and transfer-limit constraints take precedence.
   - **Forecast charge limit**: caps voluntary charging at the rate needed for B3 to reach 98% by forecast solar-end — `deficit_wh / hours_until_solar_end`, where `deficit_wh = gap_to_98% − SolarMultiplier × remaining_solar_forecast`. 0W once battery-side solar alone covers the gap (or B3 ≥98%); full once solar has ended. Forecast remaining is daily-min smoothed (running min, reset each day) for a pessimistic estimate. Transfer-limit forced absorption still applies at all SOC levels (MinCharge floor wins via lo>hi tie-break).
...
17. **tankLevelsWorker** (src/tank_levels_worker.go) - Computes water tank fill % from 5m Tukey-trimean-smoothed ADC voltages + HA calibration input_numbers; publishes powerctl-owned sensors (Water Tanks device). Invalid data (sensor offline, degenerate calibration) ⇒ no publish ⇒ entities expire to unavailable. Spec: `specs/water-tanks.md`
